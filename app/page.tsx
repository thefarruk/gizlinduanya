    ...
    <div style={{ minHeight: "100vh", background: "#07070f", color: "#EEEDFE", fontFamily: "sans-serif" }}>
      ...
      <section style={{ padding: "140px 24px 80px", textAlign: "center", maxWidth: 800, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: 16, color: "#9F9AEC", lineHeight: 1.7, marginBottom: 40 }}>
          {lang === "tk" ? "Hekaýalar. Tanyşlyk. Boýun almalar. Hemmesi anonim." : lang === "ru" ? "Истории. Знакомства. Исповеди. Всё анонимно." : lang === "tr" ? "Hikayeler. Tanışmalar. İtiraflar. Hepsi anonim." : "Stories. Connections. Confessions. All anonymous."}
        </p>
        <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
          ...
          <a href="/app/stories" style={{ border: "1px solid rgba(127,119,221,0.25)", color: "#9F9AEC", fontFamily: "monospace", fontSize: 13, padding: "14px 32px", borderRadius: 8, textDecoration: "none" }}>
            {lang === "tk" ? "Hekaýalary gör" : lang === "ru" ? "Смотреть истории" : lang === "tr" ? "Hikayelere bak" : "Browse stories"}
          </a>
        </div>
      </section>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 16, padding: "0 24px 60px", maxWidth: 1200, margin: "0 auto" }}>
        {[
          { icon: "🔥", href: "/app/stories", title: { tk: "Anonim hekaýalar", ru: "Анонимные истории", tr: "Anonim hikayeler", en: "Anonymous stories" }[lang] },
          { icon: "💬", href: "/dating", title: { tk: "Anonim tanyşlyk", ru: "Анонимные знакомства", tr: "Anonim tanışma", en: "Anonymous dating" }[lang] },
          { icon: "🤫", href: "/app/confessions", title: { tk: "Boýun almalar", ru: "Исповеди", tr: "İtiraflar", en: "Confessions" }[lang] },
          ...
          <a key={i} href={card.href} style={{ background: "#0d0d1a", border: "1px solid rgba(127,119,221,0.12)", borderRadius: 12, padding: 28, textDecoration: "none", color: "inherit", display: "block" }}>
            ...
            <h3 style={{ fontFamily: "Georgia, serif", fontSize: 20 }}>{card.title}</h3>
          </a>
        ))}
      </div>
      <footer style={{ borderTop: "1px solid rgba(127,119,221,0.1)", padding: "40px 24px", textAlign: "center" }}>
        ...
        <p style={{ fontFamily: "monospace", fontSize: 11, color: "#555470" }}>© 2025 — anonim · howpsuz · yzyz ýok</p>
      </footer>
      {modal && (
        <div onClick={() => setModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(7,7,15,0.92)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div onClick={e => e.stopPropagation()} style={{ background: "#0d0d1a", border: "1px solid rgba(127,119,221,0.25)", borderRadius: 16, padding: 40, width: "90%", maxWidth: 400, position: "relative" }}>
            ...
            <h3 style={{ fontFamily: "Georgia, serif", fontSize: 24, marginBottom: 24 }}>{modal === "login" ? "Login" : "Register"}</h3>
